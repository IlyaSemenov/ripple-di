...
type ModelNode = InputNode | FactoryNode
...
const MIN_FACTORY_COUNT = 5
const MAX_FACTORY_COUNT = 8
const MIN_FACTORY_DEPENDENCIES = 1
const MAX_FACTORY_DEPENDENCIES = 3
...
async function verifySeed(seed: number) {
  ...
  const inputCount = random.integer(MIN_INPUT_COUNT, MAX_INPUT_COUNT)
  const factoryCount = random.integer(MIN_FACTORY_COUNT, MAX_FACTORY_COUNT)
  const model: ModelNode[] = []
  const dependencies: Dependency<Box>[] = []

  for (let index = 0; index < inputCount; index += 1) {
    const defaultValue = random.integer(MIN_DEFAULT_VALUE, MAX_DEFAULT_VALUE)
    model.push({ kind: "input", defaultValue })
    dependencies.push(
      runtime.defineDependency(() => ({ value: defaultValue }), {
        name: `input-${index}`,
      }),
    )
  }

  for (let index = inputCount; index < inputCount + factoryCount; index += 1) {
    const isInitialFactory = index === inputCount
    const dependencyCount = isInitialFactory
      ? MAX_FACTORY_DEPENDENCIES
      : random.integer(
          MIN_FACTORY_DEPENDENCIES,
          Math.min(MAX_FACTORY_DEPENDENCIES, index),
        )
    const node: FactoryNode = {
      kind: "factory",
      dependencies: random.distinctIntegers(dependencyCount, 0, index - 1),
      operation: isInitialFactory
        ? "conditional"
        : valueAt(
            MODEL_OPERATIONS,
      ...
      runtime.defineDependency(
        () => ({
          value: evaluateNode(
            node,
            (dependencyIndex) =>
              valueAt(dependencies, dependencyIndex, "factory dependency")()
                .value,
          ),
        }),
        { name: `factory-${index}` },
  ...
  for (
    ...
    for (let nodeIndex = inputCount; nodeIndex < model.length; nodeIndex += 1) {
...
function createRandomScope(
  ...
  const overrideCount = random.integer(
    0,
    Math.min(MAX_OVERRIDES_PER_SCOPE, inputCount),
  )
  const inputs = random.distinctIntegers(overrideCount, 0, inputCount - 1)
  ...
  for (const input of inputs) {
    ...
    overrides.set(input, {
      id: `${seed}:${scopeIndex}:${input}`,
      value,
    })
    provisions.push(
      provide(valueAt(dependencies, input, "overridden input"), { value }),
    )
  }
...
function resolveReference(
  ...
  if (node.kind === "input") {
    ...
    for (const dependency of readValues.values()) {
      for (const [input, binding] of dependency.bindings) {
...
function evaluateNode(
  ...
  const index1 = valueAt(node.dependencies, 0, "factory dependency #1")
...
function findBinding(
  ...
  for (
    ...
    const binding = cursor.overrides.get(input)
    if (binding) {
      return binding
    }
  }
