...
- `runtime.ts` defines the Runtime contract and owns Runtime creation, Dependency definitions, callable routing, and the module-level API.
- `scope.ts` defines the Scope contract and owns override installation, async context, and lifecycle boundaries.
...
- Factories are synchronous and async factories are rejected.
  ...
  Factories may resolve Dependencies from their current Scope but cannot manage Scope context or lifecycle in their own Runtime.
- Values passed to `provide` are borrowed unless `options.dispose` transfers ownership to the receiving Scope.
  ...
  An owned-value Provision can be installed in only one Scope; borrowed-value and factory Provisions remain reusable.
  Values created by `provideFactory` are owned by the receiving Scope and use the Dependency's configured disposer.
- Disposers and their async descendants cannot resolve Dependencies or manage Scopes in the Runtime being closed.
...
- `tests/runtime.test.ts` covers Runtime APIs, isolation, and cross-Runtime boundaries.
